...
const commandsDocumentation = `[::b]TOPIC
	commands - commands allow you to execute certain actions within XyzCord
...
[::b]DESCRIPTION
	...
	doesn't persist between XyzCord sessions, it will be forgotten every
	...
	command-input. Instead XyzCord shows an extra dialog as soon as it
...
[::b]DESCRIPTION
	...
	start XyzCord, it will display the location of your configuration file
	in the splashscreen.

	Typical location on Linux:   [::b]~/.config/XyzCord/config.json[::-].
	Typical location on Windows: [::b]~/AppData/Roaming/XyzCord/config.json[::-].
	Typical location on MacOS:   [::b]~/.XyzCord/config.json[::-].
...
[::b]SETTINGS
	...
	[::b]ShortenLinks
		Determines whether XyzCord runs it's own link-shortener in order to
	...
	[::b]DesktopNotifications
		Determines whether XyzCord will try to notify the host systems using
	...
	[::b]Accounts
		...
		as profiles. Those allow you to let XyzCord know of multiple discord
	...
	[::b]ShowNicknames
		Decides whether a users nickname is displayed throughout XyzCord.`
