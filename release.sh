...
#
# RELEASE_DATE is the current date in format Year-Month-Day, since XyzCord
...
#
# Setting the XyzCord version-number to the current release date.
...
git commit version/version.go -m "Bump XyzCord versionnumber to version $RELEASE_DATE"

#
# Define executable names for later usage.
#

BIN_LINUX="XyzCord_linux_64"
BIN_DARWIN="XyzCord_darwin"
BIN_WINDOWS_64="XyzCord_64.exe"
BIN_WINDOWS_32="XyzCord_32.exe"

#
# Building XyzCord for darwin, linux and windows.
#
...
#
# EXE_HASH is sha256 of the previously built XyzCord.exe and is required
...
envsubst < XyzCord.json_template > XyzCord.json
...
wget https://github.com/5qw/XyzCord/archive/$RELEASE_DATE.tar.gz
TAR_HASH="$(sha256sum ./$RELEASE_DATE.tar.gz | cut -f 1 -d " ")"
export TAR_HASH
rm ./$RELEASE_DATE.tar.gz
envsubst < XyzCord.rb_template > XyzCord.rb
