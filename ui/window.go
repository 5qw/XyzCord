...
import (
	...
	"github.com/5qw/XyzCord/fileopen"
	"github.com/5qw/XyzCord/logging"
	"github.com/5qw/XyzCord/util/files"
	"github.com/5qw/XyzCord/util/fuzzy"
	...
	"github.com/5qw/XyzCord/scripting"
	"github.com/5qw/XyzCord/scripting/js"
	"github.com/5qw/XyzCord/shortcuts"
	"github.com/5qw/XyzCord/ui/components"
	"github.com/5qw/XyzCord/ui/shortcutdialog"
	"github.com/5qw/XyzCord/ui/tviewutil"
	"github.com/5qw/XyzCord/util/maths"
)
		...
		if shortcuts.ViewSelectedMessageImages.Equals(event) {
			...
			if targetFolder == "" {
				...
				if osError == nil && cacheDir != "" {
					...
					targetFolder = filepath.Join(cacheDir, "XyzCord")
...
Welcome to version %s of XyzCord. Below you can see the most
...
[::b]2020-08-30
	- Features
		...
		- New parameter "--account" to start XyzCord with a certain account
	- Changes
...
[::b]2020-01-05
	- Features
		...
		- XyzCord now has an amazing autocompletion
		- support for TFA
	...
	- Bugfixes
		...
		- XyzCord doesn't crash anymore when sending a message into an empty channel
...
func (window *Window) initExtensionEngine(engine scripting.Engine) error {
	...
	engine.SetTriggerNotificationFunction(func(title, text string) {
		notifyError := beeep.Notify("XyzCord - "+title, text, "assets/information.png")
		if notifyError != nil {
...
func (window *Window) handleNotification(message *discordgo.Message, channel *discordgo.Channel) error {
	...
	return beeep.Notify("XyzCord - "+notificationLocation, message.ContentWithMentionsReplaced(), "assets/information.png")
}
...
func (window *Window) toggleUserContainer() {
	...
	window.updateUserList()
	//Solves https://github.com/5qw/XyzCord/issues/246
...
// be scanned and the resulting TFA-Token can be entered into XyzCord and used
	...
	tokenInput.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		if event.Key() == tcell.KeyEnter {
			...
			successText.SetText("Setting up Two-Factor-Authentication was a success.\n\n" +
				...
				"If you need to view your backup codes again, just run `tfa backup` in the XyzCord CLI.\n\n" +
				"Currently XyzCord doesn't support applying backup codes.")
